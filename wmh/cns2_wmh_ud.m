	...
	try
		...
		for i = 1 : cns2param.n_subjs
			...
			try
					...
					case 'creating'
						if ~ismember (cns2param.lists.subjs{i,1},cns2param.lists.crtTempFailSeg)
							...
							ME = MException ('CNS2:ud:crtTempFailSeg', ...
							 '%s failed segmentation during creating templates.', cns2param.lists.subjs{i,1});
							throw (ME);
						end
				end
				...
				quant_tbl_coh (i,:) = nan_entry (cns2param,i);
		...
		fprintf ('%s : Error at either setting cns2param, organising dirs/files, or creating templates.\n', curr_cmd);
	end
